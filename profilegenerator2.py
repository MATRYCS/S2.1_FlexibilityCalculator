...
def getPVprofile(m = 6, latitude = 46.056946, longitude = 14.505751, surface_tilt = 35, surface_azimuth = 0, usehorizon = 0, nglobal = 1, outputformat='json'):
        """
        ...
        prova = 'lat=' + str(latitude) + '&lon=' + str(longitude) + '&month=' + str(m) +'&angle='+str(surface_tilt)+'&aspect='+str(surface_azimuth) +'&usehorizon=' + str(usehorizon) + '&global=' + str(nglobal) + '&localtime=0' + '&showtemperatures=1' + '&outputformat=' + str(outputformat)
        URL = URL_BASE + prova
        print(URL)
...
latitude = 46.056946
longitude = 14.505751      
PVdata = getPVprofile(m = m, latitude = latitude, longitude = longitude)
...
def solar_power_taking_account_temperature(temperature, irradiance, Wp = 5000, system_losses=0.2, NOCT=45, coef_t=-0.47/100,stc_irradiance=1000):
        """
...
#Calculate the actual PV power based on irradiance
PVpower = solar_power_taking_account_temperature(temperature, irradiance)


def get_building_profile(buildingType, year, t_m_prev):    

        # Loop through  24*4 (15 min intervals) of the day 
        for hour in range(24*4): #in this case hour is actualy 15 min interval, therefore calc_sun_position in radiation.py needs to be modified
...
house = Zone(window_area=15,
              ...
              ach_infl=0.5,
              ventilation_efficiency=0.6,
              thermal_capacitance_per_floor_area=165000, #Very light: 80 000 Light: 110 000 Medium: 165 000 Heavy: 260 000 Very heavy:370 000
              t_set_heating=20.0,
              t_set_cooling=25.0,
              max_cooling_energy_per_floor_area=-np.inf,
              max_heating_energy_per_floor_area=np.inf,
