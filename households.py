...
class Household:
    ...
    def generate(self):
        # The Yearly consumption is the normal consumption of domestic appliances as found for years in households.
        # This excludes: (PH)EV, Heat Pump, PV
        ...
        # devices
        self.Fridges = []
        self.Devices = {     "Kettle": devices.DeviceKettle(ConsumptionKettle),\
                            "Electronics": devices.DeviceElectronics(),\
                            "Cooking":    devices.DeviceCooking(),\
                            "Ventilation": devices.DeviceVentilation(ConsumptionHouseVentilation),\
                            "Ironing": devices.DeviceIroning(ConsumptionIron),\
                            "Vacuumcleaner": devices.DeviceVacuumcleaner(ConsumptionVacuumcleaner),\
        ...
        self.familyActivites = random.randint(familyOutingChanceMin, familyOutingChanceMax) / 100
        ...
        for k, v, in self.ConsumptionShare.items():
            ...
            if sumDevice != 0:
                multiplier = ((self.ConsumptionShare[k ] /100) *
                            (((self.ConsumptionYearly / 365) * numDays) * 1000) * 60) / sumDevice  # joules
            else:
                multiplier = 1
    ...
    def generateDishwashdays(self, days):
        ...
        # print("dishwash = ", self.DishwashMoment)            
    ...
    def simulate(self):
        for day in range(self.startDay, numDays + self.startDay):
            ...
            if day - self.startDay < numDays:
                ...
                if dayOfWeek in self.WashingDays:
                ...
                if self.hasDishwasher == True:
                    if dayOfWeek in self.DishwashDays:
    ...
    def saveToFile(self, num):
        text = []
        writer.writeHousehold(self, num)
...
class HouseholdSingleWorker(Household):
    def __init__(self, startDay=1):
        ...
        self.generate()
        self.ConsumptionYearly = profilegentools.gaussMinMax(2010,
                                                             400) * consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
        ...
        if random.randint(1, 2) == 1:
            ...
            self.Fridges = [devices.DeviceFridge(random.randint(ConsumptionFridgeSmallMin, ConsumptionFridgeSmallMax)),
                            devices.DeviceFridge(random.randint(ConsumptionFridgeSmallMin, ConsumptionFridgeSmallMax))]
...
class HouseholdCouple(Household):
    ...
    def __init__(self, parttime=False, jobless=False, startDay=1):
        Household.__init__(self, startDay)
        self.generate()
        self.ConsumptionYearly = profilegentools.gaussMinMax(3360,
                                                             700) * consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
...
class HouseholdFamilyDualParent(Household):
    ...
    def __init__(self, parttime=False, jobless=False, startDay=1):
        Household.__init__(self, startDay)
        self.generate()
        numKids = round(max(min(4, random.gauss(1.7, 0.4)),
                            1))  # http://www.cbs.nl/nl-NL/menu/themas/bevolking/faq/specifiek/faq-hoeveel-kinderen.htm

        self.ConsumptionYearly = profilegentools.gaussMinMax(2010 + (700 * numKids), 500 + (
                    numKids * 100)) * consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
        ...
        self.hasDishwasher = True  # random.randint(0,5) < 7 #100%

        # Determine washing days
        self.generateWashingdays(7)  # (min(5+numKids, 7)) #washing every day

        # Dermine Dishwasher times
        if self.hasDishwasher:
            self.generateDishwashdays(7)  # (min(5+numKids, 7))
...
class HouseholdFamilySingleParent(Household):
    def __init__(self, parttime=False, jobless=False, startDay=1):
        ...
        self.ConsumptionYearly = profilegentools.gaussMinMax(3360 + (700 * numKids), 500 + (
                    numKids * 100)) * consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
...
class HouseholdDualRetired(Household):
    def __init__(self, startDay=1):
        ...
        self.generate()
        self.ConsumptionYearly = profilegentools.gaussMinMax(3360,
                                                             600) * consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
